// import {ImgToInk} from '../src/img-to-ink';
import * as request from 'request-promise';
const creds = require(process.cwd() + '/auth-app/cache.json').access_token;
/**
 * @jest-environment node
 */

describe('The library can be tested!', () => {
  it.skip('can lookup a page', async () => {
    const result = await request.get(
      'https://www.onenote.com/api/v1.0/me/notes/pages/0-87a9dbf30a1d4a59881fc013b1bef299!122-8767EE548AEB0210!77130/content',
      // 'http://localhost:8000',
      {
        headers: {
      ...
      // 'http://localhost:8000',
      {
        ...
        formData: {
          presentation: {
            ...
            <html>
            <head>
            <title>Page with InkML</title>
            </head>
            </html>
            `,
          },
          'presentation-onenote-xml': {
            options: {
              contentType: 'application/inkml+xml',
              'Content-Disposition': 'form-data; name=presentation-onenote-xml',
            },
            value: `
